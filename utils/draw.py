...
def get_frustum_params(size):
  ...
  frustum_vertices = np.array([[0, 0, 0],     # 0
                                ...
                                [2, -2, -6],     # 3
                                [-2, -2, -6]])   # 4
  frustum_vertices = frustum_vertices * size

  frustum_faces = np.array([[0, 1, 2], [0, 2, 3], [0, 3, 4], [0, 4, 1], [1, 2 ,3], [1, 3, 4]])
  return frustum_edges, frustum_vertices, frustum_faces

def get_3d_axis(size):
  # OpenGL convention
  X = np.array([(0, 0, 0), (1, 0, 0)]) * size
  Y = np.array([(0, 0, 0), (0, 1, 0)]) * size
  Z = np.array([(0, 0, 0), (0, 0, 1)]) * size

  X_color = (255, 0, 0, 128)
  Y_color = (0, 255, 0, 128)
  Z_color = (0, 0, 255, 128)

  return [X, Y, Z], [X_color, Y_color, Z_color]
