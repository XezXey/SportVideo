...
def read_tracking(full_name, ext='npy'):
  if ext == 'npy':
    return np.load(full_name, allow_pickle=True)

  if ext == 'csv':
    tracking = pd.read_csv(full_name)
    return tracking.loc[:, ~tracking.columns.str.contains('^Unnamed')].loc[:, ['x', 'y']].values
